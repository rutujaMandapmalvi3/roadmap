  ...
  try {
      ...
      messages = [
        {
          role: "system",
          content:
            'You are an expert learning coach. Return a JSON object with this exact structure: { "phases": [{ "title": string, "duration": string, "milestones": [{ "title": string, "resources": [string] }] }] }',
        },
        {
          role: "user",
          content: `Topic: ${topic}. Current level: ${currentLevel}. Timeframe: ${timeframe}. Goal: ${goal}.`,
        },
      ];
    }
    ...
    if (!roadmapValidation.success) {
      return res.status(500).json({
        error: "OpenAI returned an unexpected structure. Please try again.",
      });
    }
